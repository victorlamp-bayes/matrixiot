...
def rawDataToProtocol(Object rawData) {
    ...
    String valveWarning = "0";//阀门异常告警（1异常 0 正常）
    //tag 6 周期数据
    ...
    try {
        ...
        if (JSON.isValid(rawData.toString())) {
            ...
            if (raw.containsKey("taskId") && raw.containsKey("deviceId") && raw.containsKey("result")) {
                boolean status = true;
                String message = "";
                String commandId = raw.getAt("taskId").toString();
                long timestamp = Long.parseLong(raw.getAt("timestamp").toString());
                ...
                callbackRecord.put("timestamp", timestamp);
...
//反流告警时间（1告警 0不告警）
                                    ...
                                    if (dataID.equals("03")) {
                                        valveWarning = String.valueOf(Integer.valueOf(sTemp2.substring(2, 4), 16));//阀门异常告警（1异常 0 正常）*/
                                        sTemp2 = sTemp2.substring(4);//截取长度需加上tag+length
                                    }
                                }
...
//                    root.put("battery", convertUtils.fill(String.valueOf(Math.round((Double.parseDouble(batteryQuantity) / 3.68) * 100)), 3));
                    ...
                    root.put("valveWarning", valveWarning);
        ...
        if (valveWarning.equals("1")) {
            event.put("valveWarning", valveWarning)
        }
