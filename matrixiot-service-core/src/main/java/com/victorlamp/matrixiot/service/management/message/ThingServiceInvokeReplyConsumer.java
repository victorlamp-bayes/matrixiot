...
@Component
@Slf4j
@RocketMQMessageListener(topic = ThingTopic.THING_SERVICE_INVOKE_REPLY, consumerGroup = ThingTopicConsumerGroup.DataConsumer + "_" + ThingTopic.THING_SERVICE_INVOKE_REPLY)
public class ThingServiceInvokeReplyConsumer implements RocketMQListener<ThingData> {
    ...
    public void onMessage(ThingData message) {
        // 存储设备响应的消息
        if (message == null) {
            log.error("调用设备服务MQ回复消息内容为空");
            throw new ServiceException(
                    ServiceException.ExceptionType.INTERNAL_FAILURE,
                    ExceptionInfoBuilder.build(ExceptionTemplate.INTERNAL_FAILURE_OTHER, "调用设备服务MQ回复消息内容为空"));
        }

        try {
            thingDataRepository.save(message);
        } catch (Exception e) {
            log.error("调用设备服务写入数据库失败：" + e.getMessage());
            throw new ServiceException(
                    ServiceException.ExceptionType.INTERNAL_FAILURE,
                    ExceptionInfoBuilder.build(ExceptionTemplate.INTERNAL_FAILURE_DATABASE, "调用设备服务写入"));
        }
        ...
        try {
            ...
            log.error("调用设备服务写入thingData数据库成功，更新设备状态写入thing数据库失败：" + e.getMessage());
            throw new ServiceException(
                    ServiceException.ExceptionType.INTERNAL_FAILURE,
                    ExceptionInfoBuilder.build(ExceptionTemplate.INTERNAL_FAILURE_DATABASE, "调用设备服务写入成功，更新设备状态写入数据库"));
        }
    }
}
